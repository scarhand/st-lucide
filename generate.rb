...
def load_remote_icons
  document = Nokogiri::HTML.parse(open('https://lucide.dev/'))
  document.css('button.chakra-button p.chakra-text').map(&:text)
end
...
begin
  remote_icons = load_remote_icons
  raise "Remote icon set empty" if remote_icons.empty?
  icons = remote_icons
rescue => e
  puts "Unable to load remote icons, using fallback."
end
